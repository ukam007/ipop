...
**关键改进**
- **仅删除用户输入字符**：保留提示符，使用退格删除用户输入

**技术实现**
- 字符回显阶段：立即显示服务器回显的字符
- 回车后：退格删除用户输入字符（`\b \b`），保留提示符
- 显示服务器响应：命令回显 + 结果 + 新提示符

**显示效果**
```
输入过程中：
attach_to_123$ pwd     ← 用户输入，服务器回显，立即显示

按回车后：
attach_to_123$ pwd     ← 提示符保留，用户输入删除后显示服务器命令回显
/root                  ← 命令结果
attach_to_123$         ← 新提示符
```
...
**关键改进**
- **仅保留服务器回显**：删除用户输入信息，显示完整服务器响应

**技术实现**
- 字符回显阶段：立即显示服务器回显的字符
- 回车后：删除整行（用户输入信息），显示服务器响应（不过滤命令回显）
